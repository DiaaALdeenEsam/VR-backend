...
# --- RAG evidence curation --------------------------------------------------
#
# Evidence retrieved by EvidenceRetriever (see app/domain/repositories.py) is
# raw retrieval output from an external knowledge base -- it must never reach
# this prompt unfiltered, for the same reason SYSTEM_PROMPT_TEMPLATE already
# forbids the model from revealing a diagnosis or medical terminology: a
# simulated *patient* shouldn't have access to (or be able to echo) clinical
# reasoning like lab interpretation, differential diagnosis, or a management
# plan, even indirectly via RAG-supplied background text.
#
# Content types a real patient could plausibly "know" without it being
# diagnostic reasoning -- what they feel, what's in their history, what an
# exam might show about them. "investigations"/"diagnosis"/"management"
# (and anything else) are excluded by default-deny: a patient doesn't
# reason from lab values or treatment plans.
_ALLOWED_EVIDENCE_CONTENT_TYPES = frozenset({"symptoms", "history", "exam"})
_MAX_EVIDENCE_ITEMS = 3
_MAX_EVIDENCE_CHARS_PER_ITEM = 300

_EVIDENCE_BLOCK_HEADER = (
    "\n\n[ملاحظة داخلية للمساعد فقط -- ليست جزءًا من ردّك]\n"
    "المعلومات التالية خلفية بحثية لفهمك الشخصي فقط. لا تلخّصها، ولا تقتبس منها، ولا تذكر "
    "أنها موجودة. تذكّر: أنت المريض، لست الطبيب:\n"
)
_EVIDENCE_BLOCK_FOOTER = (
    "\n[تذكير أخير قبل الرد -- أنت المريض، لست الطبيب]\n"
    "ممنوع أن تكتب جملًا مثل: \"أعتقد أنها قرحة هضمية\"، أو \"يبدو التهابًا في المعدة\"، أو "
    "\"يُفضَّل أن تراجع أخصائي جهاز هضمي\" -- هذه أمثلة على ما يُمنع قوله تمامًا.\n"
    "بدلًا من ذلك، صف شعورك فقط بلغة المريض العادي، مثل: \"أشعر بحرقة في أعلى بطني، تزداد "
    "أحيانًا ليلًا\" -- دون تسمية أي مرض أو تخصص طبي أو نصيحة علاجية."
)


def _curate_evidence(evidence: list[Evidence]) -> list[Evidence]:
    """Filters + caps raw RAG evidence down to what's safe to summarize into
    the patient's prompt. See the module-level comment above for why."""

    allowed = [e for e in evidence if e.content_type in _ALLOWED_EVIDENCE_CONTENT_TYPES]
    allowed.sort(key=lambda e: e.rank)
    return allowed[:_MAX_EVIDENCE_ITEMS]


def _format_evidence_block(evidence: list[Evidence]) -> str:
    """Renders curated evidence as an appended prompt block, or "" if there's
    nothing left after curation (in which case the prompt is unchanged from
    before RAG existed)."""

    curated = _curate_evidence(evidence)
    if not curated:
        return ""

    lines = [f"- {e.text[:_MAX_EVIDENCE_CHARS_PER_ITEM].strip()}" for e in curated]
    return _EVIDENCE_BLOCK_HEADER + "\n".join(lines) + _EVIDENCE_BLOCK_FOOTER
...
def _build_chat(
    scenario: Scenario, history: list[Message], user_message: str, evidence: list[Evidence]
) -> list[dict[str, str]]:
    system_prompt = SYSTEM_PROMPT_TEMPLATE.format(case_text=scenario.case_text) + _format_evidence_block(evidence)
...
def _generate_sync(
    ...
    try:
        ...
        chat = _build_chat(scenario, history, user_message, evidence)
...
class QwenPatientReplyGenerator(PatientReplyGenerator):
    ...
    async def generate_reply(
        ...
        return await asyncio.to_thread(_generate_sync, scenario, history, user_message, evidence or [])
